...
class ChatService {
    ...
    static async getOrCreateConversation(user1Id, user2Id) {
        // Validate ObjectIds
        if (!mongoose_1.default.Types.ObjectId.isValid(user1Id) || !mongoose_1.default.Types.ObjectId.isValid(user2Id)) {
        ...
        const user1ObjectId = new mongoose_1.default.Types.ObjectId(user1Id);
        const user2ObjectId = new mongoose_1.default.Types.ObjectId(user2Id);
        // Sort participants to ensure consistent ordering for unique index
        const participants = [user1ObjectId, user2ObjectId].sort((a, b) => a.toString().localeCompare(b.toString()));
        // Try to find existing conversation
        let conversation = await Conversation_1.Conversation.findOne({ participants })
    ...
    static async getUserConversations(userId, page = 1, limit = 20) {
        ...
        const [conversations, totalCount] = await Promise.all([
            Conversation_1.Conversation.find({ participants: userObjectId })
                .sort({ updatedAt: -1 })
                .skip(skip)
                .limit(limit)
                .populate('participants', 'name email profilePicture')
                .populate('lastMessage.sender', 'name email profilePicture'),
            Conversation_1.Conversation.countDocuments({ participants: userObjectId }),
        ]);
    ...
    static async createMessage(conversationId, senderId, content) {
        // Validate ObjectIds
        if (!mongoose_1.default.Types.ObjectId.isValid(conversationId) || !mongoose_1.default.Types.ObjectId.isValid(senderId)) {
        ...
        const conversation = await Conversation_1.Conversation.findById(conversationObjectId);
        if (!conversation) {
            throw new Error('Conversation not found');
        }
        // Validate sender is a participant
        const isParticipant = conversation.participants.some((participantId) => participantId.toString() === senderId);
        if (!isParticipant) {
    ...
    static async getConversationMessages(conversationId, userId, page = 1, limit = 50) {
        ...
        const isParticipant = conversation.participants.some((participantId) => participantId.toString() === userId);
        if (!isParticipant) {
            throw new Error('User is not a participant in this conversation');
        }
        const skip = (page - 1) * limit;
        ...
        const [messages, totalCount] = await Promise.all([
            Message_1.Message.find({ conversationId: conversationObjectId })
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limit)
                .populate('sender', 'name email profilePicture'),
            Message_1.Message.countDocuments({ conversationId: conversationObjectId }),
        ]);
        const totalPages = Math.ceil(totalCount / limit);
        return {
            messages,
     ...
     */
    static async updateMessageStatus(messageId, status, userId) {
        // Validate ObjectId
     ...
     */
    static async markMessagesAsRead(conversationId, userId, messageIds) {
        // Validate ObjectIds
        if (!mongoose_1.default.Types.ObjectId.isValid(conversationId) || !mongoose_1.default.Types.ObjectId.isValid(userId)) {
            throw new Error('Invalid conversation or user ID format');
        }
        ...
        const result = await Message_1.Message.updateMany(query, {
            $set: {
                status: 'seen',
                seenAt: now,
     ...
     */
    static async updateConversationLastMessage(conversationId, senderId, content, createdAt) {
        // Validate ObjectIds
        if (!mongoose_1.default.Types.ObjectId.isValid(conversationId) || !mongoose_1.default.Types.ObjectId.isValid(senderId)) {
            throw new Error('Invalid conversation or sender ID format');
        }
        ...
        await Conversation_1.Conversation.findByIdAndUpdate(conversationObjectId, {
            $set: {
                lastMessage: {
    ...
    static async isConversationParticipant(conversationId, userId) {
        ...
        return conversation.participants.some((participantId) => participantId.toString() === userId);
    }
    /**
     ...
     */
    static getMessageTickDisplay(messageStatus, isRecipientOnline) {
     ...
     */
    static async getMessageWithTickDisplay(messageId, isRecipientOnline) {
        // Validate ObjectId
        if (!mongoose_1.default.Types.ObjectId.isValid(messageId)) {
            throw new Error('Invalid message ID format');
        }
